...
from utils.utils import get_date_range_days  # Import the same utility function
...
def get_search_params():
    ...
    try:
        ...
        # Validate date parameter - FIXED: Use same logic as search_routes.py
        ...
        if date_posted_days is not None:
            ...
            if date_posted_days < 0:
                ...
                date_range = None  # No filter for invalid values
            elif date_posted_days > 365:
                log_security_event("INVALID_DATE_RANGE", f"Date days too large: {date_posted_days}")
                date_posted_days = 365
                date_range = None  # No filter for excessive values
            elif date_posted_days >= 31:
                # If >= 31 days, treat as "everything" (no date filter)
                date_posted_days = 365
                date_range = None
            else:
                # For values < 31, create actual date range
                date_range = get_date_range_days(date_posted_days)
        else:
            # Default case - no date filter specified
            date_posted_days = 365
            date_range = None
        ...
        return {
            ...
            'date_range': date_range,  # Add the processed date_range
            'offset': offset,
            'limit': limit
        }
